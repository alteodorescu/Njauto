// EthVpVwapStrategyInverse
// ------------------------
// EXACT MIRROR of EthVpVwapStrategy with one transformation: every entry
// signal produced by TryFindSetup() is inverted before sizing/execution.
//   - IsLong is flipped
//   - StructuralTarget is reflected across EntryPrice so the R-multiple
//     and SL geometry remain identical to the non-inverted setup
//
// All other behavior - regime classification, propfirm guard, scaling,
// historical levels, plots, journal - is byte-for-byte identical to the
// base strategy. Run them side-by-side to compare baseline vs inversion
// without disturbing the existing build.
//
// Place this file next to EthVpVwapStrategy.cs:
//   Documents/NinjaTrader 8/bin/Custom/Strategies/EthVpVwapStrategyInverse.cs
// Reuses the same AddOns/NjAuto/*.cs library.
...
{
    public class EthVpVwapStrategyInverse : Strategy
    {
        // ----- Engines -----
        ...
        public double TickValueOverride { get; set; } = 1.25;
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = "INVERTED ETH VP/VWAP strategy. Every base-strategy signal is direction-flipped and target-reflected.";
                Name = "EthVpVwapStrategyInverse";
            ...
            else if (State == State.DataLoaded)
            {
                ...
                guard.Transition += (prev, next, reason) =>
                    Print(string.Format("[Guard][INV] {0} -> {1} : {2}", prev, next, reason));
                ...
                if (!string.IsNullOrWhiteSpace(JournalDir))
                {
                    try
                    {
                        ...
                        journalCsvPath = Path.Combine(JournalDir,
                            string.Format("njauto_inverse_{0}_{1:yyyyMMdd_HHmmss}.csv",
        ...
        protected override void OnBarUpdate()
        {
            ...
            // Detect a base setup, then INVERT before sizing/execution.
            TradeSignal sig = TryFindSetup();
            sig = InvertSignal(sig);
            if (sig == null) return;

            TryEnter(sig);
        }

        // The single substantive change vs the base strategy: flip direction
        // and reflect the structural target across entry. R/SL math stays
        // symmetric so MinRMultiple compares identical distances.
        private TradeSignal InvertSignal(TradeSignal sig)
        {
            if (sig == null) return null;
            double reflectedTarget = 2.0 * sig.EntryPrice - sig.StructuralTarget;
            return new TradeSignal
            {
                Kind = sig.Kind,
                IsLong = !sig.IsLong,
                EntryPrice = sig.EntryPrice,
                StructuralTarget = reflectedTarget,
                Reason = "[INV] " + sig.Reason
            };
        }

        private void OnEthSessionStart(DateTime sessionStartLocal)
        {
            Print("=== [INV] ETH session start: " + sessionStartLocal);
            profile.Reset();
            vwap.Reset();
            regime.Reset();
        ...
        private void OnEthSessionEnd(DateTime sessionEndLocal)
        {
            ...
            Print("=== [INV] ETH session end: " + sessionEndLocal +
        ...
        private void UpdatePlots()
        {
            ...
            string label = string.Format("[INV] Regime: {0} ({1})   {2}",
            ...
            Draw.TextFixed(this, "njauto_inv_regime", label,
        ...
        private void UpdateHistoricalLevelPlots()
        {
            ...
            {
                string tag = string.Format("histlvl_inv_{0:yyyyMMdd}_{1}",
        ...
        {
            if (!ShowChartPlots) return;
            string tag = "njauto_inv_entry_" + CurrentBar;
            ...
            Draw.Text(this, tag + "_lbl", string.Format("[INV] {0} {1}c", sig.Kind, contracts),
                0,
        ...
        private TradeSignal TryFindHistoricalLevelSetup()
        {
            ...
            if (Math.Abs(distanceToVwap) < meta.TickSize) return null;
        ...
        private void TryEnter(TradeSignal sig)
        {
            ...
            else
            {
                if (FixedContracts <= 0 || FixedSlTicks < MinSlTicks)
                {
                    if (VerboseLog) Print(string.Format(
                        "[Skip][INV] Fixed sizing invalid: contracts={0}, slTicks={1} (min {2})",
            ...
            if (!decision.TakeTrade)
            {
                ...
                {
                    Print("[Skip][INV] " + decision.Reason);
            ...
            if (tpDistance < MinRMultiple * slDistance)
            {
                ...
                {
                    Print(string.Format(
                        "[Skip][INV] TP below {0}R floor (most recent: {1:F2} vs {2:F2})",
            ...
            // Use a distinct signal-name prefix so this strategy's brackets
            // never collide with the base strategy's if both are loaded.
            activeSignal = "INV_" + sig.Kind.ToString();
            ...
            Print(string.Format(
                "[Enter][INV] {0} {1}c @ ~{2:F2}  SL={3:F2}({4}t)  TP={5:F2}  reason={6}",
